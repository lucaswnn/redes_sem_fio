...
namespace ns3
{
    ...
    ns3::TypeId
    VlcChannel::GetTypeId(void) // returns meta-information about VlcErrorModel class
    {                           // including parent class, group name, constructor, and attributes
        static ns3::TypeId tid = ns3::TypeId("VlcChannel")
                                     .SetParent<ns3::Channel>()
                                     .AddConstructor<VlcChannel>()
                                     .AddAttribute("AveragePower",
                                                   "Average Power",
                                                   DoubleValue(0),
                                                   MakeDoubleAccessor(&VlcChannel::m_AvgPower),
    ...
    void
    ns3::VlcChannel::Attach(Ptr<VlcNetDevice> device)
    {
    ...
    std::size_t
    VlcChannel::GetNDevices(void) const
    {
        return 2;
    }
    ...
    void
    VlcChannel::SetPropagationLossModel(ns3::Ptr<ns3::PropagationLossModel> loss)
    {
    ...
    VlcChannel::GetPropagationLossModel()
    {
        NS_LOG_FUNCTION(this);
        return this->m_loss;
    }
    ...
    void
    VlcChannel::SetPropagationDelayModel(ns3::Ptr<ns3::PropagationDelayModel> delay)
    {
    ...
    VlcChannel::GetPropagationDelayModel()
    {
        NS_LOG_FUNCTION(this);
        return this->m_delay;
    }
    ...
    void
    VlcChannel::SetDistance()
    {
        NS_LOG_FUNCTION(this);
        ns3::Ptr<VlcNetDevice> first = ns3::DynamicCast<VlcNetDevice>(this->GetDevice(0));
        ns3::Ptr<VlcNetDevice> second = ns3::DynamicCast<VlcNetDevice>(this->GetDevice(1));

        ns3::Ptr<VlcPropagationLossModel> l = ns3::DynamicCast<VlcPropagationLossModel>(this->m_loss);
        this->m_distanceBWTXandRX =
            l->GetDistance(first->GetMobilityModel(), second->GetMobilityModel());
    }

    void
    VlcChannel::DoCalcPropagationLoss()
    {
        NS_LOG_FUNCTION(this);
    ...
    void
    VlcChannel::SetPropagationDelay(double delay)
    {
        NS_LOG_FUNCTION(this << delay);
        // this->m_delay->set
    }
    ...
    double
    VlcChannel::DoCalcPropagationLossForSignal(int timeInstant)
    {
        NS_LOG_FUNCTION(this);
        ns3::Ptr<VlcTxNetDevice> first = ns3::DynamicCast<VlcTxNetDevice>(this->GetDevice(0));
        ns3::Ptr<VlcRxNetDevice> second = ns3::DynamicCast<VlcRxNetDevice>(this->GetDevice(1));
        double pMax = *std::max_element(first->GetTXOpticalPowerSignal().begin(),
                                        first->GetTXOpticalPowerSignal().end());
        double pMin = *std::min_element(first->GetTXOpticalPowerSignal().begin(),
                                        first->GetTXOpticalPowerSignal().end());
    ...
    double
    VlcChannel::GetDistance(ns3::Ptr<ns3::MobilityModel> aTX, ns3::Ptr<ns3::MobilityModel> bRX) const
    {
        double dist = 0;
    ...
    void
    VlcChannel::SetWavelength(int lower, int upper)
    { // sets upper and lower bound wavelength [nm]
        NS_LOG_FUNCTION(this);
        this->m_SNR->SetWavelength(lower, upper);
    }

    void
    VlcChannel::SetTemperature(double t)
    ...
    void
    VlcChannel::SetReceivedPower(double p)
    ...
    void
    VlcChannel::CalculateSNR()
    { // caluclates the SNR value
        NS_LOG_FUNCTION(this);
    ...
    { // returns the signal-to-noise ratio (SNR)
        ...
        auto rxMob = rx->GetMobilityModel();
        auto txPos = txMob->GetPosition();
        auto rxPos = rxMob->GetPosition();
        auto vLOS = rxPos - txPos;
        auto vLOSDist = vLOS.GetLength();

        // vetor unitário entre tx e rx
        vLOS = ns3::Vector(vLOS.x / vLOSDist, vLOS.y / vLOSDist, vLOS.z / vLOSDist);

        // vetor normal ao LOS do rx
        auto rxNorm = ns3::Vector(std::cos(rxMob->GetElevation()) * std::cos(rxMob->GetAzimuth()),
                                  std::cos(rxMob->GetElevation()) * std::sin(rxMob->GetAzimuth()),
    ...
    void
    VlcChannel::SetAveragePower(double power)
    {
    ...
    void
    VlcChannel::SetElectricNoiseBandWidth(double b)
    ...
    double
    VlcChannel::GetNoiseBandwidth()
    { // return the noise bandwidth
        NS_LOG_FUNCTION(this);
        return m_SNR->GetNoiseBandwidth();
    }

    bool
    VlcChannel::TransmitStart(Ptr<Packet> p, Ptr<VlcNetDevice> src, Time txTime)
    {
        ...
        rx->GetErrorModel()->SetSNR(
            this->GetSNR()); // for each transmission event calculate the SNR and set it in error model.
    ...
    void
    VlcChannel::TransmitDataPacket(Ptr<Packet> p)
    {
        NS_LOG_FUNCTION(this << p);
