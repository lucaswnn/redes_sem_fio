...
namespace ns3
{
	...
	double VlcPropagationLossModel::GetResp() const
	{
		return this->m_RXGain * this->m_TXGain * this->m_FilterGain * this->m_ConcentratorGain;
	}
												  ...
												  ns3::Ptr<ns3::MobilityModel> bRX) const
	{
		...
		double resp = this->GetResp();

		auto txMob = ns3::DynamicCast<ns3::VlcMobilityModel>(aTX);
		auto rxMob = ns3::DynamicCast<ns3::VlcMobilityModel>(bRX);
		...
		// vetor normal ao LOS do tx
		auto txNorm = ns3::Vector(std::cos(txMob->GetElevation()) * std::cos(txMob->GetAzimuth()),
								  std::cos(txMob->GetElevation()) * std::sin(txMob->GetAzimuth()),
								  std::sin(txMob->GetElevation()));

		// cosseno do ângulo de irradiância
		auto cosIrr = vLOS.x * txNorm.x + vLOS.y * txNorm.y + vLOS.z * txNorm.z;
		...
		double pRX = txPowerDbm * this->m_Area * resp * (cosIrr / M_PI) * cosInc;
