...
namespace ns3
{
  ...
  public:
    ...
    virtual std::size_t GetNDevices(
        void) const; // gets the number of devices attached to the channel.

    virtual Ptr<NetDevice> GetDevice(
        std::size_t i) const; // gets the device specidied by the index number.

    void SetPropagationLossModel(ns3::Ptr<ns3::PropagationLossModel>
                                     loss); // sets the propagation loss model for the channel

    ns3::Ptr<ns3::PropagationLossModel> GetPropagationLossModel(); // returns a pointer to the
                                                                   // propagation loss model

    void SetPropagationDelayModel(ns3::Ptr<ns3::PropagationDelayModel>
                                      delay); // sets the propagation delay model for the channel

    ns3::Ptr<ns3::PropagationDelayModel> GetPropagationDelayModel(); // returns the propagation
                                                                     // delay model of the channel

    void SetPropagationDelay(double delay);

    double GetDistance(); // returns the distance after computing the distance using the mobility
                          // models of TX and RX

    void SetDistance(); // sets the distance after computing the distance using the mobility models
                        // of TX and RX

    void DoCalcPropagationLoss(); // calculates the optical power signal for the RX using the
                                  // optical power signal of the TX and loss model

    double DoCalcPropagationLossForSignal(
        int timeInstant); // computes the optical power signal of RX at a particular instant
    ...
    bool TransmitStart(Ptr<Packet> p,
                       Ptr<VlcNetDevice> src,
                       Time txTime); // starts the transmission of packets.

    double GetDistance(ns3::Ptr<ns3::MobilityModel> aTX,
                       ns3::Ptr<ns3::MobilityModel> bRX)
        const; // returns the distance between sender and receiver devices.
