...
ns3::TypeId
VlcMobilityModel::GetTypeId(void)
{
    static ns3::TypeId tid =
        ns3::TypeId("ns3::VlcMobilityModel")
...
Ptr<MobilityModel>
VlcMobilityModel::Copy() const
{
    Ptr<VlcMobilityModel> copy = CreateObject<VlcMobilityModel>();

    copy->m_baseTime = m_baseTime;
    copy->m_basePosition = m_basePosition;
    copy->m_baseVelocity = m_baseVelocity;
    copy->m_acceleration = m_acceleration;
    copy->m_azimuth = m_azimuth;
    copy->m_elevation = m_elevation;
...
VlcMobilityModel::DoGetVelocity(void) const
{
    ...
    return Vector(m_baseVelocity.x + m_acceleration.x * t,
                  ...
                  m_baseVelocity.z +
                      m_acceleration.z * t); // returns a velocity vector of the node that has this
                                             // mobility model instlled on it
}
...
VlcMobilityModel::DoGetPosition(void) const
{
    ...
    return Vector(m_basePosition.x + m_baseVelocity.x * t + m_acceleration.x * half_t_square,
                  ...
                  m_basePosition.z + m_baseVelocity.z * t +
                      m_acceleration.z *
                          half_t_square); // returns a acceleration vector of the node that has this
                                          // mobility model instlled on it
}

void
VlcMobilityModel::DoSetPosition(
    const Vector& position) // called to set position via vector parameters
{
...
void
VlcMobilityModel::SetVelocityAndAcceleration(
...
void
VlcMobilityModel::SetAzimuth(double angle)
{
    m_azimuth = angle * (M_PI / 180);
}

void
VlcMobilityModel::SetElevation(double angle)
{
    m_elevation = angle * (M_PI / 180);
}

double
VlcMobilityModel::GetAzimuth(void)
{
    return m_azimuth;
}

double
VlcMobilityModel::GetElevation(void)
{
    return m_elevation;
}
