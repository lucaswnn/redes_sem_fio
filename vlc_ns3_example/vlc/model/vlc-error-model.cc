    ...
    static TypeId tid =
        TypeId("ns3::VlcErrorModel")
            ...
            .AddAttribute("Beta",
                          ...
                          DoubleValue(0.0),
                          MakeDoubleAccessor(&VlcErrorModel::beta),
                          MakeDoubleChecker<double>())
            .AddAttribute("SNR",
                          ...
                          MakeDoubleChecker<double>())
            .AddAttribute("RanVar1",
                          "The decision variable attached to this error model.",
                          StringValue("ns3::UniformRandomVariable[Min=0.0|Max=1.0]"),
                          MakePointerAccessor(&VlcErrorModel::m_ranvar),
                          MakePointerChecker<RandomVariableStream>())
            .AddAttribute("ModulationScheme",
                          "The modulation scheme used in this model",
                          EnumValue(PAM4),
                          MakeEnumAccessor<VlcErrorModel::ModScheme>(&VlcErrorModel::mod_scheme),
...
VlcErrorModel::ModScheme
VlcErrorModel::GetScheme(void) const // returns the modulation scheme used
{
    NS_LOG_FUNCTION(this);
    return mod_scheme;
}
...
void
VlcErrorModel::SetScheme(ModScheme scheme) // sets the value of the modulation scheme used
{
...
void
VlcErrorModel::SetRandomVariable(Ptr<RandomVariableStream> ranvar)
{
...
int64_t
VlcErrorModel::AssignStreams(int64_t stream)
{
...
int
VlcErrorModel::GetModulationOrder(void) const
{
    NS_LOG_FUNCTION(this);
    return mod_order;
}
...
void
VlcErrorModel::SetModulationOrder(int m_order)
{
...
double
VlcErrorModel::GetAlpha(void) const
{
    NS_LOG_FUNCTION(this);
    return alpha;
}
...
void
VlcErrorModel::SetAlpha(double a)
{
...
double
VlcErrorModel::GetBeta(void) const
{
    NS_LOG_FUNCTION(this);
    return beta;
}
...
void
VlcErrorModel::SetBeta(double b)
{
...
double
VlcErrorModel::GetSNR(void) const
{
    NS_LOG_FUNCTION(this);
    return SNR;
}
...
void
VlcErrorModel::SetSNR(double snr)
{
...
double
VlcErrorModel::CalculateErrorRate(void)
{
    NS_LOG_FUNCTION(this);
    if (mod_scheme == VPPM)
    {
    ...
    else if (mod_scheme == PAM4 || mod_scheme == OOK || mod_scheme == PAM8 ||
             mod_scheme == PAM16) // OOK IS A SUBSET OF PAM
    {
        return CalculateSER();
    }
...
double
VlcErrorModel::CalculateSER(void)
{
    NS_LOG_FUNCTION(this);
...
double
VlcErrorModel::CalculateBER(void)
{
    NS_LOG_FUNCTION(this);
...
bool
VlcErrorModel::IsCorrupt(Ptr<Packet> p)
{
    ...
    if (mod_scheme == PAM4 || mod_scheme == OOK || mod_scheme == PAM8 || mod_scheme == PAM16)
    {
        ...
        double symbols_per_pkt =
            static_cast<double>(8 * p->GetSize()) / symbol_size; // no. of symbols per packet
...
double
VlcErrorModel::GenerateQfunction(double a)
{
    // double d = a*100 - 155;
    double d = a;
...
bool
VlcErrorModel::CorruptPacket(Ptr<Packet> p, double ber)
{
...
double
VlcErrorModel::GetAveragePower(double pmax, double pmin, double alpha)
{
    if (mod_scheme == VPPM)
    {
        return alpha * pmax;
