...
namespace ns3
{
	...
	{							   // including parent class, group name, constructor, and attributes
		static TypeId tid =
			TypeId("ns3::VlcSnr").SetParent<Object>().SetGroupName("Network").AddConstructor<VlcSnr>().AddAttribute("PowerReceived", "average received optical signal power", DoubleValue(0), MakeDoubleAccessor(&VlcSnr::Pr), MakeDoubleChecker<double>()).AddAttribute("SNR", "signal-to-noise-ratio", DoubleValue(0), MakeDoubleAccessor(&VlcSnr::SNR), MakeDoubleChecker<double>()).AddAttribute("LowerWavelength", "lower bound WaveLength", IntegerValue(0), MakeIntegerAccessor(&VlcSnr::wavelength_lower), MakeIntegerChecker<int>()).AddAttribute("UpperWavelength", "upper bound WaveLength", IntegerValue(0), MakeIntegerAccessor(&VlcSnr::wavelength_upper), MakeIntegerChecker<int>()).AddAttribute("Temperature", "Blackbody temp of LED", DoubleValue(0), MakeDoubleAccessor(&VlcSnr::temp), MakeDoubleChecker<double>());
		return tid;
	}
	...
	void VlcSnr::CalculateNoiseVar(double A, double resp, double ambientNoisePower)
	{
		...
		// shot noise variance
		shot_var = (2 * q * resp * 1.25 * Pr * B) + (2 * q * Ib * I2 * B);
	...
	void VlcSnr::SetElectricNoiseBandWidth(double b)
	{ // sets the noise bandwidth
		NS_LOG_FUNCTION(this);
		B = b;
	}

	double VlcSnr::GetNoiseBandwidth()
	...
	double VlcSnr::SpectralRadiance(int wavelength, double temperature)
	{
		...
		spectral_rad =
			15 * (std::pow((h * c) / (M_PI * k * temperature), 4)) / ((std::pow(waveLength, 5)) * ((std::exp((h * c) / (waveLength * k * temperature))) - 1));
		return spectral_rad;
	}
