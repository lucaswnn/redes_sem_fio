...
namespace ns3
{
    ...
    void
    VlcChannelHelper::CreateChannel(std::string channelName)
    {
        // NS_LOG_FUNCTION(this);
    ...
    void
    VlcChannelHelper::SetChannelParameter(std::string chName, std::string paramName, double value)
    {
        if (paramName == "TEMP")
        {
    ...
    void
    VlcChannelHelper::SetPropagationLoss(std::string channelName, std::string propagationLossType)
    {
        // NS_LOG_FUNCTION(this);
    ...
    void
    VlcChannelHelper::SetPropagationDelay(std::string channelName, double value)
    {
        // NS_LOG_FUNCTION(this);
    ...
    double
    VlcChannelHelper::GetChannelSNR(std::string chName)
    {
        ns3::Ptr<VlcRxNetDevice> rx = DynamicCast<VlcRxNetDevice>(m_channel[chName]->GetDevice(1));
    ...
    double
    VlcChannelHelper::GetChannelAmbientNoisePower(std::string chName)
    {
        return this->m_channel[chName]->GetAmbientNoisePower();
    }

    void
    VlcChannelHelper::SetChannelAmbientNoisePower(std::string chName, double ambientNoisePower)
    {
        this->m_channel[chName]->SetAmbientNoisePower(ambientNoisePower);
    }

    void
    VlcChannelHelper::SetChannelWavelength(std::string channelName, int lower, int upper)
    {
        this->m_channel[channelName]->SetWavelength(lower, upper);
    }

    void
    VlcChannelHelper::AttachTransmitter(std::string chName,
                                        std::string TXDevName,
                                        ns3::Ptr<VlcDeviceHelper> devHelper)
    {
        // NS_LOG_FUNCTION(this);
        // this->m_channel[chName]->Attach(devHelper->GetTransmitter(TXDevName));
        ns3::Ptr<VlcPropagationLossModel> loss =
            ns3::DynamicCast<VlcPropagationLossModel>(m_channel[chName]->GetPropagationLossModel());
    ...
    void
    VlcChannelHelper::AttachReceiver(std::string chName,
                                     std::string RXDevName,
                                     ns3::Ptr<VlcDeviceHelper> devHelper)
    {
        // NS_LOG_FUNCTION(this);
        ...
        loss->SetConcentratorGain(devHelper->GetReceiver(RXDevName)->GetConcentrationGain());
    ...
    ns3::Ptr<VlcChannel>
    VlcChannelHelper::GetChannel(std::string chName)
    {
        NS_LOG_FUNCTION(this);
        return m_channel[chName];
    }

    ns3::Ptr<VlcNetDevice>
    VlcChannelHelper::GetDevice(std::string chName, uint32_t idx)
    {
        NS_LOG_FUNCTION(this);
        ns3::Ptr<VlcNetDevice> ans = DynamicCast<VlcNetDevice>(m_channel[chName]->GetDevice(idx));
        return ans;
    }

    NetDeviceContainer
    VlcChannelHelper::Install(std::string chName, Ptr<Node> a, Ptr<Node> b)
    {
        NS_LOG_FUNCTION(this);
        NetDeviceContainer container;
        Ptr<VlcChannel> ch = this->m_channel[chName];

        Ptr<VlcTxNetDevice> devTX = DynamicCast<VlcTxNetDevice>(ch->GetDevice(0));
        Ptr<VlcRxNetDevice> devRX = DynamicCast<VlcRxNetDevice>(ch->GetDevice(1));
    ...
    NetDeviceContainer
    VlcChannelHelper::Install(Ptr<Node> a, Ptr<Node> b, Ptr<VlcTxNetDevice> tx, Ptr<VlcRxNetDevice> rx)
    {
        NetDeviceContainer container;
    ...
    NetDeviceContainer
    VlcChannelHelper::Install(Ptr<Node> a,
                              Ptr<Node> b,
                              Ptr<VlcDeviceHelper> devHelper,
                              Ptr<VlcChannelHelper> chHelper,
                              std::string txName,
                              std::string rxName,
                              std::string chName)
    {
