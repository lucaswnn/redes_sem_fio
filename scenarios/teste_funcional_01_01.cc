...
#define HALF_POW_ANGLE_DEG 60
#define PHOTO_RECV_AREA 1
#define PHOTO_RECV_RESP 1
#define ELECTRON_LOAD 1
...
struct PhyTransmissionData
{
    double halfPowAngle;
    double photoRecvArea;
    double photoRecvResp;
    ...
    double capacitance;
    double voltageGain;
    double channelNoiseFactor;
    double circuitNoise;
    double paramMisterioso;
};

struct GeometricTransmissionData
{
    double phi;
    double theta;
    double dist;
...
double RadEmit(PhyTransmissionData phyData,
               GeometricTransmissionData geomData)
{
    m = std::log(0.5) / std::log(std::cos(phyData.halfPowAngle));

    return (m + 1) * std::pow(std::cos(geomData.phi)) / (2 * M_PI);
}

double Attenuation(PhyTransmissionData phyData,
                   GeometricTransmissionData geomData)
{
    return RadEmit(geomData.phi) *
           phyData.photoRecvArea *
           std::cos(geomData.theta) /
           (geomData.dist * geomData.dist);
}

double SigmaDisp(PhyTransmissionData p,
                 GeometricTransmissionData g)
{
    double recvPower = Attenuation(p, g) * p.transmissionPower;

    return 2 * p.electronLoad *
           (p.photoRecvResp * recvPower *
                (1 + p.modulationIndex * p.modulationIndex) +
            p.bgCurrent * p.noiseFactor) *
           p.noiseBandwidth;
}

double SigmaTerm(hyTransmissionData p,
                 GeometricTransmissionData g)
{
    return 8.0 * M_PI * p.boltzmannConst * p.absTemperature *
           p.capacitance * p.photoRecvArea * p.noiseBandwidth * p.noiseBandwidth *
           (p.noiseFactor / p.voltageGain +
            2 * M_PI * p.channelNoiseFactor * p.capacitance *
                p.photoRecvArea * p.circuitNoise *
                p.noiseBandwidth / p.paramMisterioso);
}
...
double ComputeBerFromSnr(double snrDb)
{
    ...
    double ber = Qfunc(std::sqrt(2.0 * snrLinear)); // por que 2.0?
